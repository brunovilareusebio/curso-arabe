...
const courseData = {
    ...
    modulos: [
        ...
        },
        {
            ...
            links: [
                {nome: "Arabic Greetings", url: "https://www.youtube.com/watch?v=dQw4w9WgXcQ"},
        ...
        },
        {
            ...
            receita: {
                ...
                preparo: "Prato egipcio: fava cozida com temperos, servida no café da manhã"
            },
            links: [
...
class ArabicCourseApp {
    ...
    createModuleCard(modulo, index) {
        ...
        card.onclick = () => !isLocked && this.abrirModulo(modulo.id);
    ...
    abrirModulo(moduleId) {
        ...
        title.textContent = `Módulo ${modulo.id}: ${modulo.titulo}`;
        progressFill.style.width = `${this.userProgress.moduleProgress[moduleId] || 0}%`;

        body.innerHTML = this.renderModuleContent(modulo);
    ...
    renderModuleContent(modulo) {
        return `
            <div class="module-content">
                ...
                <div class="module-section">
                    <button class="btn btn--primary" onclick="app.concluirModulo(${modulo.id})">
                        ✓ Concluir Módulo
                    </button>
                </div>
            </div>
    ...
    renderFlashcard() {
        ...
        frontText.textContent = currentWord.arabe;
        backText.textContent = currentWord.portugues;
        counter.textContent = `${this.currentFlashcardIndex + 1}/${vocabulario.length}`;
    ...
    fecharFlashcards() {
        document.getElementById('flashcards-modal').classList.remove('active');
    }
    ...
    renderQuizQuestion() {
        ...
        questionText.textContent = quiz.pergunta;
        scoreElement.textContent = this.quizScore;
        totalElement.textContent = courseData.quizzes.length;
        ...
        quiz.opcoes.forEach((opcao, index) => {
            ...
            button.onclick = () => this.selecionarOpcao(index);
    ...
    finalizarQuiz() {
        ...
        feedback.className = percentage >= 70 ? 'quiz-feedback correct' : 'quiz-feedback incorrect';
        feedback.innerHTML = `
        ...
        nextBtn.onclick = () => this.abrirQuiz();
    }
    ...
    fecharCaligrafia() {
        document.getElementById('caligrafia-modal').classList.remove('active');
    }

    // Sistema de Diálogo com IA
    abrirDialogo() {
    ...
    gerarRespostaIA(userMessage) {
        const respostas = [
            ...
            "النطق الصحيح هو...<br>A pronúncia correta é...",
    ...
    fecharDialogo() {
        document.getElementById('dialogo-modal').classList.remove('active');
    }
    ...
    fecharGlossario() {
        document.getElementById('glossario-modal').classList.remove('active');
    }
    ...
    initializeAudio() {
        // Simular funcionalidade de áudio
        this.currentAudioWord = 0;
    }
    ...
    renderAudioPlayer() {
        ...
        wordElement.textContent = currentWord.arabe;
        transliterationElement.textContent = currentWord.transliteracao;
        
        practiceList.innerHTML = vocabulario.slice(0, 12).map((word, index) => `
            <div class="audio-practice-item" onclick="app.selecionarPalavraAudio(${index})">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Suporte para Enter no campo de busca do glossário
    ...
    // Suporte para Enter no campo de diálogo
